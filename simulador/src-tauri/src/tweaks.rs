...
// MODO SIMULACIÓN: estado en memoria de qué tweaks están "aplicados", para que
// los interruptores sean coherentes al navegar. No se toca el sistema.
...
/// [SIMULACIÓN] No aplica nada; solo recuerda el estado en memoria.
#[tauri::command(async)]
pub fn apply_tweak(id: String) -> Result<String, String> {
    let tw = find(&id)?;
    APPLIED.lock().unwrap().get_or_insert_with(HashSet::new).insert(id);
    Ok(format!("[Simulación] Aplicado: {}", tw.title))
}

/// [SIMULACIÓN] No revierte nada; solo actualiza el estado en memoria.
#[tauri::command(async)]
pub fn revert_tweak(id: String) -> Result<String, String> {
    let tw = find(&id)?;
    APPLIED.lock().unwrap().get_or_insert_with(HashSet::new).remove(&id);
    Ok(format!("[Simulación] Revertido: {}", tw.title))
}

/// [SIMULACIÓN] Devuelve el estado en memoria de cada tweak.
#[tauri::command(async)]
pub fn check_all_tweaks() -> Result<Value, String> {
    let guard = APPLIED.lock().unwrap();
    let empty = HashSet::new();
    let set = guard.as_ref().unwrap_or(&empty);
    let map: serde_json::Map<String, Value> = catalog()
        .into_iter()
        .map(|tw| (tw.id.to_string(), Value::Bool(set.contains(tw.id))))
        .collect();
    Ok(Value::Object(map))
}
